...
function run_sim(raw_string::AbstractString; L=2^5,U=1,Re=250,mem=Array)
    ...
    gif_path = sim_gif!(sim,duration=1,clims=(-10,10),plotbody=true)

    # Open the generated GIF automatically
    try
        if Sys.iswindows()
            run(`cmd /c start "" "$gif_path"`)
        elseif Sys.isapple()
            run(`open $gif_path`)
        else
            run(`xdg-open $gif_path`)
        end
        println("Opened GIF viewer for: $gif_path")
    catch e
        println("Could not automatically open GIF viewer: $e")
        println("GIF saved at: $gif_path")
    end

    return gif_path
end

function make_sim(raw_string::AbstractString; L=2^5,U=1,Re=100,mem=Array)
